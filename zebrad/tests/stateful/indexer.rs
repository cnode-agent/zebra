...
async fn has_spending_transaction_ids() -> Result<()> {
    ...
    // Read the last 500 blocks, which is greater than the 100 non-finalized blocks
    // retained by the test cache, so both finalized and non-finalized state are checked.
